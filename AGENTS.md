...
Shubham owns this. Start: say hi + 1 motivating line.
Work style: telegraph; noun-phrases ok; drop grammar; min tokens.

## Agent Protocol
- Contact: Shubham Rastogi (@SentientRamen12, shu.rastogi337@gmail.com).
- Workspace: `~/Desktop/repos`. Missing SentientRamen12 repo: clone `https://github.com/SentientRamen12/<repo>.git`.
- 3rd-party/OSS (non-SentientRamen12): clone under `~/Desktop/repos/oss`.
- `~/Desktop/repos/manager`: private ops (domains/DNS, redirects/workers, runbooks).
- “MacBook” / “Mac Studio” => SSH there; find hosts/IPs via `tailscale status`.
- Files: repo or `~/Desktop/repos/agent-scripts`.
...
- Oracle: run `npx -y @SentientRamen12/oracle --help` once/session before first use.
...
- Remotes under `~/Desktop/repos`: prefer HTTPS; flip SSH->HTTPS before pull/push.
...
Read `~/Desktop/repos/agent-scripts/tools.md` for the full tool catalog if it exists.
...
### xcodegen
- Generates Xcode Desktop/repos from YAML specs; run `xcodegen --help`.
...
- Run `npx -y @SentientRamen12/oracle --help` once/session (before first use).
